...
print("🌿 BEXIA v48 DEFINITIVA iniciando...", flush=True)
OWNER_SECRET="BEXIA_FER_2026_INFINITA_SUPREMA"
app=FastAPI(title="BEXIA v48", docs_url=None, redoc_url=None, openapi_url=None)
...
def cerebro(user_text, sid):
    ...
    if t in ["ok","okay","si","sí","dale","gracias","chau","hola","buenas"]:
        if t in ["hola","buenas"]: return "¡Hola! Soy Bexia v48. Clima real con % lluvia, búsqueda adaptativa y memoria persistente. ¿En qué te ayudo?"
        return "¡De una! ¿Seguimos con otro tema o clima?"
    ...
    if "aprende que" in t:
        ...
        return f"Anotado en pendientes ID {item['id']}: '{hecho[:120]}'. Fer debe aprobar en /admin."
    q=user_text.strip()
    ...
    if len(q)>=2:
        ...
        if r:
            ...
            if wiki: return wiki[:950] + f"\n\n[Motor: Wikipedia | Orden: {stats['orden']}]"
    ...
    for k,v in BASE_OFFLINE.items():
        if k in t:
            ...
            save_json("bexia_ajustes.json",ajustes)
            return v
    return f"Sobre '{user_text[:60]}' te explico simple o técnico, decime como querés."

@app.get("/")
def root(): return {"bexia":"v48 DEFINITIVA","orden":stats.get("orden"),"efectividad":ajustes.get("efectividad")}

@app.get("/app", response_class=HTMLResponse)
def app_public():
    html="""<html><head><meta name=viewport content="width=device-width,initial-scale=1"><title>BEXIA v48</title><style>*{margin:0;padding:0;box-sizing:border-box}body{background:#0b0b12;color:#fff;font-family:system-ui;height:100vh;display:flex;flex-direction:column}header{padding:12px;background:linear-gradient(90deg,#7c3aed,#06b6d4);font-weight:900;display:flex;justify-content:space-between}#chat{flex:1;overflow:auto;padding:14px;display:flex;flex-direction:column;gap:10px}.msg{max-width:85%;padding:12px 14px;border-radius:18px;font-size:14px;white-space:pre-wrap}.user{background:#7c3aed;align-self:flex-end}.bexia{background:#1a1a2e;border:1px solid #2a2a4a;align-self:flex-start}.composer{padding:10px;background:#0f0f1a;display:flex;gap:8px}input{flex:1;padding:13px;border-radius:999px;background:#1a1a2e;border:1px solid #333;color:#fff}button{padding:13px 20px;border-radius:999px;background:linear-gradient(90deg,#7c3aed,#06b6d4);border:none;color:#fff;font-weight:900}.footer{padding:8px;text-align:center;font-size:11px;color:#888}</style></head><body><header><div>BEXIA v48 DEFINITIVA</div><div style="font-size:11px;background:rgba(255,255,255,.2);padding:4px 10px;border-radius:999px">Clima+Adaptativa+Memoria</div></header><div id=chat></div><div class=composer><input id=inp placeholder="Clima Chivilcoy, que es PLC, aprende que..."><button onclick=enviar()>></button></div><div class=footer>Bexia v48 - Hecha por Fer</div><script>const sid='u'+Math.random().toString(36).slice(2,9);const chat=document.getElementById('chat');const inp=document.getElementById('inp');function add(t,c){const d=document.createElement('div');d.className='msg '+c;d.textContent=t;chat.appendChild(d);chat.scrollTop=chat.scrollHeight;return d;}async function enviar(){const txt=inp.value.trim();if(!txt)return;add(txt,'user');inp.value='';const th=add('Pensando...','bexia');try{const r=await fetch('/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:txt,session_id:sid})});const j=await r.json();th.textContent=j.respuesta;}catch(e){th.textContent='Error conexion.';}}inp.addEventListener('keydown',e=>{if(e.key==='Enter')enviar();});add("Hola! Soy Bexia v48 DEFINITIVA. Clima con % lluvia, busqueda adaptativa que aprende, memoria persistente. ¿Qué probamos?","bexia");</script></body></html>"""
...
async def chat_endpoint(req: ChatRequest, request: Request):
    ...
    try:
        ...
        if prioridad_ordenada!=ajustes["prioridad"]:
            ...
            save_json("bexia_ajustes.json",ajustes)
        return {"respuesta": r, "orden":stats.get("orden"), "efectividad":ajustes["efectividad"]}
