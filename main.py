"""
BEXIA v38 MAIN COMPLETO ADAPTATIVO - Busca SIEMPRE, nunca se rinde
- Usa las libretas completas adaptativas
- 4 motores + memoria + calculadora
- Fix 2+2= + clima Chivilcoy + que es PLC todo en uno
"""
...
# Buscador adaptativo - funciona con requests o urllib
try:
    import requests
    HAS_REQUESTS=True
except:
...
app = FastAPI(title="BEXIA v38 ADAPTATIVA", docs_url=None, redoc_url=None, openapi_url=None)
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_methods=["*"], allow_headers=["*"])

def load_json(p,d):
    try:
        if os.path.exists(p):
            with open(p,"r",encoding="utf-8") as fp: return json.load(fp)
    except: pass
    return d
def save_json(p,d):
    try:
